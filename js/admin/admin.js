...
async function init() {
  app.innerHTML =
    '<main class="login-card"><p>Verifying secure access…</p></main>';
...
function shell(content) {
  ...
  app
    ...
    .forEach((b) => (b.onclick = () => show(b.dataset.section)));
...
async function show(section) {
  ...
  if (section === "Business Information" || section === "Site Settings")
...
async function loadReferences(config) {
  const needed = config.fields
    .filter((f) => f[2] === "reference")
    .map((f) => f[4].source);
...
async function manager(name) {
  ...
  if (!config) {
    shell('<div class="notice">This content area is not configured.</div>');
    return;
  }
  ...
  if (error) {
    shell(
      `<div class="notice">Could not load content: ${esc(error.message)}</div>`,
    );
    return;
  }
  records = data || [];
...
function renderManager(name, config, extraHtml = "") {
  ...
  app
    .querySelectorAll("[data-delete]")
    .forEach(
      (b) => (b.onclick = () => removeRecord(name, config, b.dataset.delete)),
    );
}
...
function wireBlogCategoryBar() {
  ...
  app.querySelectorAll("[data-remove-category]").forEach(
    (b) =>
      (b.onclick = async () => {
        if (
          !confirm(
            "Delete this category? Posts using it will need a new category.",
          )
        )
          return;
        const { error } = await client
          .from("blog_categories")
          .delete()
          .eq("id", b.dataset.removeCategory);
        if (error) {
          notice(error.message, true);
          return;
        }
...
function openEditor(name, config, record) {
  const dialog = app.querySelector("#editor"),
    editing = Boolean(record.id);
  pendingImages = {};
  const guidance =
    name === "Testimonials"
      ? '<div class="editor-guidance"><strong>Publishing tip</strong><span>Published reviews appear on the Testimonials page and service pages. “Feature on homepage” also adds a review to the homepage.</span></div>'
      : "";
  ...
  dialog.querySelector("#record-form").onsubmit = (e) =>
    saveRecord(e, name, config, record.id);
...
function field([key, label, type, required, extra], record) {
  ...
  if (type === "list")
    return `<label class="wide">${label}<textarea name="${key}" ${required ? "required" : ""}>${esc(Array.isArray(value) ? value.join("\n") : value)}</textarea><small>Enter one item per line.</small></label>`;
  if (type === "faq")
    return `<label class="wide">${label}<textarea name="${key}" ${required ? "required" : ""}>${esc(Array.isArray(value) ? value.map((item) => `${item.question || ""} | ${item.answer || ""}`).join("\n") : value)}</textarea><small>Use one line per FAQ, separating the question and answer with a vertical bar.</small></label>`;
  if (type === "textarea")
    return `<label class="wide">${label}<textarea name="${key}" ${required ? "required" : ""}>${esc(value)}</textarea></label>`;
  if (type === "select")
    return `<label>${label}<select name="${key}" ${required ? "required" : ""}>${statuses.map((s) => `<option value="${s}" ${value === s ? "selected" : ""}>${title(s)}</option>`).join("")}</select></label>`;
  if (type === "checkbox")
    return `<label class="check"><input type="checkbox" name="${key}" ${value ? "checked" : ""}> ${label}</label>`;
  const formatted =
...
function wireImageFields(root) {
  root.querySelectorAll("[data-image-field]").forEach((wrap) => {
    const key = wrap.dataset.key,
      bucket = wrap.dataset.bucket,
      maxMB = Number(wrap.dataset.max) || 5;
...
async function saveRecord(event, name, config, id) {
  ...
  config.fields
    .filter((f) => f[2] === "checkbox")
    .forEach((f) => (values[f[0]] = form.elements[f[0]].checked));
  config.fields
    .filter((f) => f[2] === "number")
    .forEach(
      (f) => (values[f[0]] = values[f[0]] === "" ? null : Number(values[f[0]])),
    );
  config.fields
    .filter((f) => f[2] === "datetime-local")
    .forEach(
      (f) =>
        (values[f[0]] = values[f[0]]
          ? new Date(values[f[0]]).toISOString()
          : null),
    );
  config.fields
    .filter((f) => f[2] === "reference")
    .forEach((f) => {
      if (values[f[0]] === "") values[f[0]] = null;
    });
  config.fields
    .filter((f) => f[2] === "list")
    .forEach(
      (f) =>
        (values[f[0]] = String(values[f[0]] || "")
          .split(/\r?\n/)
          .map((v) => v.trim())
          .filter(Boolean)),
    );
  ...
  status.textContent = Object.keys(pendingImages).length
    ? "Uploading image…"
    : "Saving…";
  const existingRecord = id ? records.find((r) => r.id === id) : null;
  const { error: imageError } = await resolveImages(
    values,
    config,
    existingRecord,
  );
  if (imageError) {
    ...
    status.textContent = `Image upload failed: ${imageError.message}`;
    return;
  }
  status.textContent = "Saving…";
  const query = id
    ? client.from(config.table).update(values).eq("id", id)
    : client.from(config.table).insert(values);
...
async function removeRecord(name, config, id) {
  ...
  if (
    !confirm(
      `Delete "${record?.[config.titleField] || "this record"}"? This cannot be undone.`,
    )
  )
    return;
...
function renderGalleryManager(kind, cfg) {
  ...
  app
    ...
    .forEach(
      (b) =>
        (b.onclick = () => removeGalleryRecord(kind, cfg, b.dataset.delete)),
    );
...
function renderGalleryDesignControls(cfg) {
  const defaults = {
      studio: "/assets/images/gallery-studio-architecture.webp",
      ritual: "/assets/images/gallery-ritual-still-life.webp",
      water: "/assets/images/gallery-water-glass.webp",
    },
    labels = {
      studio: "Studio architecture",
      ritual: "Wellness ritual",
      water: "Water & glass detail",
    };
  const panel = document.createElement("section");
  panel.className = "panel gallery-design-panel";
  panel.innerHTML = `<div class="panel-head"><div><p class="kicker">Built-in gallery artwork</p><h2>Default image design</h2><p>These images show when no published gallery records exist. Replace any or all of them here.</p></div><button class="primary" id="edit-gallery-design">Replace images</button></div><div class="gallery-design-preview">${Object.entries(
    ...
    .map(
      ([slot, fallback]) =>
        `<figure><img src="${galleryDesign[slot] ? publicUrl(client, cfg.bucket, galleryDesign[slot]) : fallback}" alt=""><figcaption>${labels[slot]}${galleryDesign[slot] ? "<span>Custom</span>" : "<span>ENIVÈ default</span>"}</figcaption></figure>`,
    )
    .join("")}</div>`;
  app.querySelector(".manager-intro")?.after(panel);
  panel.querySelector("#edit-gallery-design").onclick = () =>
    openGalleryDesignEditor(cfg, defaults, labels);
}

function openGalleryDesignEditor(cfg, defaults, labels) {
  const dialog = app.querySelector("#editor");
  pendingImages = {};
  dialog.innerHTML = `<form class="editor-form" id="gallery-design-form"><div class="editor-head"><div><p class="kicker">Gallery appearance</p><h2>Replace default images</h2></div><button class="icon-button" type="button" data-close aria-label="Close">×</button></div><div class="editor-guidance"><strong>How this works</strong><span>Choose a new image for any slot. Leave a slot unchanged to keep its current image, or remove a custom image to restore the ENIVÈ default.</span></div><div class="form-grid">${Object.keys(
    defaults,
  )
    .map((slot) =>
      imageField(slot, labels[slot], galleryDesign[slot] || "", {
        bucket: cfg.bucket,
        maxMB: 8,
      }),
    )
    .join(
      "",
    )}</div><div class="editor-actions"><button type="button" data-close>Cancel</button><button class="primary" type="submit">Save image design</button></div><p role="status" aria-live="polite"></p></form>`;
  dialog.showModal();
  dialog
    .querySelectorAll("[data-close]")
    .forEach((b) => (b.onclick = () => dialog.close()));
  wireImageFields(dialog);
  dialog.querySelector("form").onsubmit = (e) => saveGalleryDesign(e, cfg);
...
async function saveGalleryDesign(event, cfg) {
  ...
  Object.keys(pendingImages).forEach((key) => {
    if (!pseudoConfig.fields.some((field) => field[0] === key))
      pseudoConfig.fields.push([
        key,
        "",
        "image",
        false,
  ...
  dialogClose(form);
  await galleryManager("gallery");
  notice("Default gallery images updated.");
}
function dialogClose(form) {
  form.closest("dialog")?.close();
}
...
function openGalleryEditor(kind, cfg, record) {
  ...
  const imageFields =
    imageField(
      "image_path",
      kind === "before_after" ? "Before image" : "Image",
      record.image_path || "",
      { bucket: cfg.bucket, maxMB: 8 },
    ) +
    (kind === "before_after"
      ? imageField(
          "after_image_path",
          "After image",
          record.after_image_path || "",
          { bucket: cfg.bucket, maxMB: 8 },
        )
      : "");
  ...
  wireImageFields(dialog);
  dialog.querySelector("#record-form").onsubmit = (e) =>
    saveGalleryRecord(e, kind, cfg, record.id, record);
  dialog.querySelector("input")?.focus();
}
...
async function saveGalleryRecord(event, kind, cfg, id, existingRecord) {
  ...
  if (cfg.needsConsent)
    values.consent_confirmed = form.elements.consent_confirmed.checked;
  ...
  if (
    ...
    status.textContent =
      "Please confirm client consent before publishing before-and-after images.";
    return;
  }
  button.disabled = true;
  status.textContent = Object.keys(pendingImages).length
  ...
  const pseudoConfig = {
    fields: [
      ["image_path", "", "image", false, { bucket: cfg.bucket }],
      ["after_image_path", "", "image", false, { bucket: cfg.bucket }],
    ],
    titleField: "title",
  };
  const { error: imageError } = await resolveImages(
  ...
  if (!id && !values.image_path) {
    button.disabled = false;
    status.textContent = "Please choose an image before saving.";
    return;
  }
  status.textContent = "Saving…";
  const query = id
    ? client.from("gallery_items").update(values).eq("id", id)
...
async function removeGalleryRecord(kind, cfg, id) {
  ...
  if (
    !confirm(
      `Delete "${record?.title || record?.category || "this item"}"? This cannot be undone.`,
    )
  )
    return;
  if (record?.image_path)
    await removeImage(client, cfg.bucket, record.image_path);
  if (record?.after_image_path)
    await removeImage(client, cfg.bucket, record.after_image_path);
...
async function inbox() {
  ...
  if (error) {
    shell(
      `<div class="notice">Could not load messages: ${esc(error.message)}</div>`,
    );
    return;
  }
  records = data || [];
...
function renderInbox() {
  ...
  app.querySelectorAll("[data-toggle-read]").forEach(
    (b) =>
      (b.onclick = async () => {
        const record = records.find((r) => r.id === b.dataset.toggleRead);
        const { error } = await client
          .from("contact_submissions")
          .update({ is_read: !record.is_read })
          .eq("id", record.id);
        if (error) {
          notice(error.message, true);
          return;
  ...
  app.querySelectorAll("[data-delete-message]").forEach(
    (b) =>
      (b.onclick = async () => {
  ...
  const fields =
    section === "Business Information"
      ...
      : [
          ...
          ["home_eyebrow", "Homepage eyebrow"],
          ["home_heading", "Homepage main heading"],
          ["home_heading_accent", "Homepage accent heading"],
          ["home_intro", "Homepage introduction"],
          ["services_eyebrow", "Services-page eyebrow"],
          ["services_heading", "Services-page main heading"],
          ["services_heading_accent", "Services-page accent heading"],
          ["services_intro", "Services-page introduction"],
          ["footer_kicker", "Footer eyebrow"],
          ["footer_heading", "Footer main heading"],
          ["footer_heading_accent", "Footer accent heading"],
        ];
  return fields
    .map(
      ([k, l]) =>
        `<label class="${k.includes("description") || k.includes("intro") ? "wide" : ""}">${l}${k.includes("description") || k.includes("intro") ? `<textarea name="${k}">${esc(v[k] || "")}</textarea>` : `<input name="${k}" value="${esc(v[k] || "")}">`}</label>`,
    )
    .join("");
}

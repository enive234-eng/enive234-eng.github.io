...
export function validateImage(file, maxMB = 5) {
  if (!ALLOWED_TYPES[file.type]) return 'Please choose a JPEG, PNG or WebP image.';
  if (file.size > maxMB * 1024 * 1024) return `Images must be ${maxMB}MB or smaller.`;
  return null;
}
...
function buildFilename(baseName, file) {
  ...
  const unique = (crypto.randomUUID ? crypto.randomUUID() : String(Date.now())).slice(0, 8);
  return `${slugify(baseName)}-${unique}.${ext}`;
}

export async function uploadImage(client, bucket, file, { baseName, folder, oldPath } = {}) {

...
const SOLO_SELECTORS = [
  ...
  ".hero-image",
  ".section-head",
  ".provider-public-profile>img",
  ".provider-public-profile>div",
  ...
  ".dynamic-page .content-shell>h2",
  ".contact-grid>div",
  ".contact-form",
  ".services-directory-head",
  ".service-guidance>div",
];
...
function tagSolo(el) {
  ...
  if (!/\breveal(-scale|-left|-right)?\b/.test(el.className)) {
    const directional = el.matches(
      ".provider-card,.provider-public-profile>img,.contact-grid>div,.detail-aside",
    )
      ? "reveal-left"
      : el.matches(
            ".provider-copy,.provider-public-profile>div,.contact-form,.detail-hero-mark",
          )
        ? "reveal-right"
        : "reveal";
...
const PARALLAX_SELECTOR =
  ".hero-premium .hero-image img, .editorial-image img, " +
...
function initLiveRescan() {
  const targets = ["#main", "#site-footer"]
